...
module Language.Optix.Utils.Pretty2 
    (
    -- * Class
      Pretty (..)
    -- * Combinators
...
-- | Class for converting a document's style into a Vty Attribute
-- No helpers are provided since this is pretty low-level
class StyleVty a where
    vtyStyle :: a -> Vty.Attr
...
instance StyleVty Void where
    vtyStyle = absurd
instance StyleVty Vty.Attr where
    vtyStyle = id
