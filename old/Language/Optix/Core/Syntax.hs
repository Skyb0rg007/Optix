        ...
        where
            ...
            pp d = \case
                ...
                ExpLetRec binds body -> do
                    let bindings = IntMap.fromList (map (\(Bound.Name.Name nm i) -> (i, nm)) (concatMap Bound.bindings binds ++ Bound.bindings body))
                        nameOf idx = IntMap.findWithDefault "_" idx bindings
                        names = map nameOf [ 0 .. length binds - 1 ]
